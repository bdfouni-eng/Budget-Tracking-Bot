    ...
    try:
        ...
        with _onedrive_upload_lock:
            ...
            try:
                ...
                resp = requests.post(session_url, headers=session_headers, json=session_body)
                ...
                for attempt in range(1, max_retries + 1):
                    try:
                        ...
                        put_resp = requests.put(upload_url, headers=put_headers, data=data_bytes)
...
# After wb.save(...) and wb.close()
time.sleep(3.0)  # gives the filesystem a moment before network upload
...
# Optional: track a single background retry future so we do not queue many
_bg_sync_future = None
    ...
    try:
        ...
        if response.status_code in (200, 201):
            ...
            if "busy" in error_msg.lower() or "423" in error_msg or "same name" in error_msg.lower():
                ...
                if _bg_sync_future is None or _bg_sync_future.done():
                    try:
...
def _upload_backup_temp_copy() -> Tuple[bool, str]:
    ...
    try:
        ...
        pr = requests.put(upload_url, headers=put_headers, data=data)
...
def add_transaction_smart(
    ...
    try:
        ...
        with excel_lock:
            ...
            sheet[f'E{row}'].value = transaction_type  # Always "Expenses"
            ...
            # Default USD = 0 if nothing was specified
            if not currency_amounts:
        ...
        if save_success:
            ...
            # ===== FIX APPLIED (D-3): clean, contiguous HTML message with no stray split =====
            message = (
...
def upload_to_different_name_then_move() -> Tuple[bool, str]:
    ...
    try:
        ...
        move_headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
        move_body = {
            "name": original_path.name,
            "@microsoft.graph.conflictBehavior": "replace"
        }
        
        move_resp = requests.patch(move_url, headers=move_headers, json=move_body)
