...
def delete_transaction_at_row(row: int) -> Tuple[bool, str, Optional[Dict]]:
    """Delete a transaction at a specific row using openpyxl."""
    try:
        # 1) Copy fresh file from OneDrive
        success, msg = copy_excel_from_onedrive()
        if not success:
        ...
        with excel_lock:
            ...
            # 3) Capture deleted transaction for feedback
            deleted_transaction = {
                "row": row,
...
def delete_last_transaction() -> Tuple[bool, str, Optional[Dict]]:
    """Delete the last transaction (last non empty row)."""
    try:
        success, msg = copy_excel_from_onedrive()
        if not success:
            return False, f"❌ {msg}", None
        ...
        with excel_lock:
            ...
            # Find the first empty row scanning from 12 upward
            last_row = 12
            while sheet[f"C{last_row}"].value not in [None, ""]:
                last_row += 1
                if last_row > 1000:
...
async def delete_command(update, context):
    ...
    /delete 125            → ask to confirm deleting row 125
    /delete 125 confirm    → delete row 125
    """
    if not is_authorized(update.effective_user.id):
        await update.message.reply_text("⛔ Unauthorized.")
        return
    ...
    # No args → show recent with rows
    if not args:
        recents = get_recent_transactions(10)
        if not recents:
            await update.message.reply_text("❌ No transactions found.")
            return
        lines = ["🧾 Last 10 transactions (row shown on the left):"]
        for t in recents:
            date_str = t["date"].strftime("%Y-%m-%d") if hasattr(t["date"], "strftime") else str(t["date"])
            item = t.get("subcategory") or t.get("category", "")
            lines.append(f"• Row {t['row']}: {date_str} — {item}")
        lines.append("\nTo delete the last one:  /delete last confirm")
        lines.append("To delete a specific row: /delete <row> confirm")
        await update.message.reply_text("\n".join(lines))
        return
    ...
    try:
        ...
        await update.message.reply_text("❌ Invalid row. Example: /delete 125 confirm")
        return

    if len(args) >= 2 and args[1] == "confirm":
        ok, msg, deleted = delete_transaction_at_row(row)
        await update.message.reply_text(msg)
    else:
        await update.message.reply_text(f"Type:  /delete {row} confirm  to delete that row.")
...
async def modify_command(update, context):
    """Usage examples:
    /modify 125 item="Daouk Sweets"
    /modify 125 usd=10.5 payment=Card
    /modify 125 lbp=200000 notes="Promo coupon"
    /modify 125 item=Chamsin euro=-5
    """
    if not is_authorized(update.effective_user.id):
        await update.message.reply_text("⛔ Unauthorized.")
        return
    ...
    if not context.args:
        help_text = (
            "How to use /modify:\n"
            "/modify <row> item=\"New Name\" usd=12.5 lbp=100000 euro=-3 payment=Card notes=\"any text\"\n\n"
            "You can change one or many fields in one command.\n"
            "Tip: run /recent to see row numbers."
        )
