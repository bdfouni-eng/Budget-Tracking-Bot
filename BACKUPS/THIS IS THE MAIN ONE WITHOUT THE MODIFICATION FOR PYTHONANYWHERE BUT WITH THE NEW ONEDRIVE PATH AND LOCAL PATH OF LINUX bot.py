...
try:
    ...
    print("⚠️  xlwings not found. Excel operations will be simulated.")
...
def export_to_csv(time_range: str = "month") -> Tuple[bool, str]:
    ...
    try:
        ...
        with excel_operation() as app:
            wb = app.books.open(str(LOCAL_COPY_PATH))
            sheet = wb.sheets[TRACKING_SHEET_NAME]
            ...
            while True:
                date_val = sheet.range(f'C{row}').value
                ...
                transaction = {
                    'date': date_val,
                    'payment': sheet.range(f'D{row}').value or "Cash",
                    'type': sheet.range(f'E{row}').value or "Expenses",
                    'category': sheet.range(f'F{row}').value or "",
                    'subcategory': sheet.range(f'G{row}').value or "",
                    'usd': sheet.range(f'H{row}').value,
                    'lbp': sheet.range(f'I{row}').value,
                    'euro': sheet.range(f'J{row}').value,
                    'notes': sheet.range(f'K{row}').value or ""
                }
        ...
        for trans in transactions:
            ...
            csv_lines.append(
                f'"{date_str}","{trans["payment"]}","{trans["type"]}","{trans["category"]}",'
                f'"{trans["subcategory"]}",{trans["usd"] or ""},{trans["lbp"] or ""},'
                f'{trans["euro"] or ""},"{trans["notes"].replace(chr(34), "\"\"") if trans["notes"] else ""}"'
            )
...
def export_summary() -> Tuple[bool, str]:
    ...
    try:
        ...
        with excel_operation() as app:
            ...
            while True:
                if sheet.range(f'C{row}').value is None:
                    break
                
                # Get category
                category = sheet.range(f'F{row}').value
                if category:
                    ...
                    # Add amounts
                    usd_val = sheet.range(f'H{row}').value or 0
                    lbp_val = sheet.range(f'I{row}').value or 0
                    euro_val = sheet.range(f'J{row}').value or 0
...
@contextmanager
def excel_operation():
    """Context manager for Excel operations with robust cleanup"""
    app = None
    try:
        with excel_lock:
            if not XLWINGS_AVAILABLE:
                raise ImportError("xlwings is not available")
        ...
        if app:
            try:
                ...
                # Last resort - kill Excel process
                try:
...
def unlock_excel_file():
    """Try to unlock the Excel file if it's locked"""
    try:
        if LOCAL_COPY_PATH.exists():
            # Try to rename and delete
            temp_path = LOCAL_COPY_PATH.with_suffix('.locked')
            try:
                LOCAL_COPY_PATH.rename(temp_path)
                time.sleep(0.5)
                if temp_path.exists():
                    os.remove(temp_path)
                logger.info("Unlocked Excel file")
                return True
            except:
                pass
        
        # Check if file is actually locked by trying to open it
        try:
            with open(LOCAL_COPY_PATH, 'a'):
                pass
            return True
        except:
            return False
...
def excel_operation_with_timeout(timeout_seconds=30):
    """Excel operation with timeout to prevent hanging"""
    import threading
    ...
    if thread.is_alive():
        logger.error(f"Excel operation timed out after {timeout_seconds} seconds")
        return None
    elif error[0]:
        raise error[0]
    else:
...
async def async_excel_operation():
    """Async context manager for Excel operations"""
    app = None
    ...
    try:
        ...
        with ThreadPoolExecutor() as executor:
            app = await loop.run_in_executor(
                executor, 
                lambda: xw.App(visible=False) if XLWINGS_AVAILABLE else None
            )
...
def copy_excel_from_onedrive() -> Tuple[bool, str]:
    """Copy from OneDrive to local with better error handling"""
    max_retries = 3
        ...
        try:
            ...
            if LOCAL_COPY_PATH.exists():
                ...
                try:
                    ...
                    logger.warning(f"Backup creation failed, continuing anyway: {backup_error}")
                
                # Try to remove the file if it exists
                try:
                    if LOCAL_COPY_PATH.exists():
                        os.remove(LOCAL_COPY_PATH)
                        logger.info(f"Removed existing local copy")
                        time.sleep(1)  # Give OS time to release the file
                except PermissionError:
                    if attempt < max_retries - 1:
                        logger.warning(f"File locked, retrying in {retry_delay} seconds...")
                        time.sleep(retry_delay)
                        continue
                    else:
                        return False, f"❌ Cannot access file after {max_retries} attempts. File may be open in Excel."
            
            # Copy the file
            shutil.copy2(ONEDRIVE_PATH, LOCAL_COPY_PATH)
            
            # Verify the copy was successful
            if LOCAL_COPY_PATH.exists() and LOCAL_COPY_PATH.stat().st_size > 0:
                logger.info(f"File copied successfully (attempt {attempt + 1})")
                return True, "✅ File copied successfully"
            else:
                return False, "❌ File copy failed (empty or missing)"
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(f"Failed to copy after {max_retries} attempts: {e}")
                return False, f"❌ Copy error: File is locked. Please close Excel and try again."
        except Exception as e:
            logger.error(f"Unexpected copy error: {e}")
            return False, f"❌ Copy error: {str(e)}"
    
    return False, "❌ Copy failed after multiple attempts"
...
def load_all_tables_with_details() -> Dict[str, Dict]:
    ...
    if not XLWINGS_AVAILABLE:
        logger.error("xlwings is not available. Cannot load tables.")
        return {}
    ...
    try:
        ...
        with excel_operation() as app:
            wb = app.books.open(str(LOCAL_COPY_PATH))
            sheet = wb.sheets[DROPDOWN_SHEET_NAME]
            ...
            for i in range(1, list_objects.Count + 1):
                ...
                if table.DataBodyRange:
                    ...
                    for row in range(1, data_range.Rows.Count + 1):
                        for col in range(1, data_range.Columns.Count + 1):
...
def add_transaction_smart(subcategory_input: str, currency_amounts: Dict[str, Optional[float]], 
    ...
    try:
        ...
        if not XLWINGS_AVAILABLE:
            return False, "❌ xlwings not installed. Please install with: pip install xlwings"
        ...
        with excel_operation() as app:
            ...
            row = 12
            while sheet.range(f'C{row}').value not in [None, ""]:
                row += 1
                if row > 1000:
                    break
            ...
            sheet.range(f'C{row}').value = today
            sheet.range(f'D{row}').value = payment_type
            sheet.range(f'E{row}').value = transaction_type  # FIXED: Always "Expenses"
            sheet.range(f'F{row}').value = category
            sheet.range(f'G{row}').value = matched_original
            ...
            for currency, amount in currency_amounts.items():
                if currency in CURRENCIES and amount is not None:
                    ...
                    sheet.range(f'{column}{row}').value = float(amount)
            
            # If no currencies specified, add to USD as default
            if not currency_amounts:
                sheet.range(f'H{row}').value = 0.0
            ...
            if optional_notes:
                ...
                sheet.range(f'K{row}').value = clean_notes
            else:
                # Leave notes cell empty if no notes provided
                sheet.range(f'K{row}').value = None
            # ========== END NOTES SECTION ==========
...
def delete_transaction_at_row(row: int) -> Tuple[bool, str, Optional[Dict]]:
    ...
    try:
        if not XLWINGS_AVAILABLE:
            return False, "❌ xlwings not installed", None
        ...
        with excel_operation() as app:
            ...
            deleted_transaction = {
                'row': row,
                'date': sheet.range(f'C{row}').value,
                'payment': sheet.range(f'D{row}').value or "Cash",
                'type': sheet.range(f'E{row}').value or "Expenses",
                'category': sheet.range(f'F{row}').value or "",
                'subcategory': sheet.range(f'G{row}').value or "",
            ...
            for col in ['C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K']:
                sheet.range(f'{col}{row}').value = None
...
# Update the existing delete_last_transaction function to use the new function
def delete_last_transaction() -> Tuple[bool, str, Optional[Dict]]:
    ...
    try:
        ...
        with excel_operation() as app:
            ...
            last_row = 12
            while sheet.range(f'C{last_row}').value not in [None, ""]:
                last_row += 1
    ...
    try:
        ...
        with excel_operation() as app:
            ...
            if new_payment_type is not None:
                sheet.range(f'D{row}').value = new_payment_type
            ...
            if changes_made:
                existing_notes = sheet.range(f'K{row}').value or ""
                ...
                if "[Modified" not in existing_notes:
                    ...
                    sheet.range(f'K{row}').value = new_notes_value
...
async def start_command(update, context):
    ...
    xlwings_status = "✅ INSTALLED" if XLWINGS_AVAILABLE else "❌ NOT INSTALLED"
    
    await update.message.reply_text(
        f"💰 <b>Smart Budget Tracker Bot</b>\n\n"
        f"📊 <b>Status:</b> {xlwings_status}\n"
...
async def cleanup_command(update, context):
    """Emergency cleanup of locked files"""
    logger.info(f"Received /cleanup from user {update.effective_user.id}")
    
    if not is_authorized(update.effective_user.id):
        await update.message.reply_text("⛔ Unauthorized.")
        return
    
    await update.message.reply_text("🧹 Running emergency cleanup...")
    ...
    try:
        ...
        if result.returncode == 0 or "not found" in result.stdout:
            ...
            if LOCAL_COPY_PATH.exists():
                try:
                    os.remove(LOCAL_COPY_PATH)
                    msg = "✅ Cleanup successful. Locked files removed."
                except:
                    msg = "⚠️ Excel processes killed but files may still be locked."
            else:
                msg = "✅ Excel processes terminated."
        else:
            msg = "✅ No Excel processes found running."
        
        await update.message.reply_text(msg)
        
    except Exception as e:
        await update.message.reply_text(f"❌ Cleanup error: {str(e)[:200]}")

async def unlock_command(update, context):
    """Emergency unlock Excel file"""
    if not is_authorized(update.effective_user.id):
        await update.message.reply_text("⛔ Unauthorized.")
        return
    ...
    try:
        ...
        if os.path.exists(LOCAL_COPY_PATH):
            try:
                ...
                locked = True
                msg = "⚠️ File still locked. Try restarting computer."
        
        # Also check for backup files
        backup_locked = False
        try:
            backups = list(BACKUP_DIR.glob("*.xlsm"))
            for backup in backups[:3]:  # Check recent backups
                try:
                    os.rename(backup, backup)  # Test if locked
                except:
                    backup_locked = True
        except:
            pass
        ...
        if locked:
            msg += "\n\n💡 **Solutions:**"
            msg += "\n1. Restart computer (recommended)"
            msg += "\n2. Open Task Manager → End EXCEL.EXE"
...
async def recent_command(update, context):
    ...
    if not XLWINGS_AVAILABLE:
        await update.message.reply_text("❌ xlwings not installed. Cannot read Excel file.")
        return
    ...
    try:
        ...
        with excel_operation() as app:
            ...
            for row in range(start_row, last_row):
                ...
                if date_val:
                    payment = sheet.range(f'D{row}').value or "Cash"
                    transaction_type = sheet.range(f'E{row}').value or "Expenses"
                    category = sheet.range(f'F{row}').value or ""
                    subcategory = sheet.range(f'G{row}').value or ""
...
async def stats_command(update, context):
    ...
    try:
        ...
        if XLWINGS_AVAILABLE:
            ...
            total_transactions = "N/A (xlwings not installed)"
        ...
        await update.message.reply_text(
            ...
            f"• <b>Excel Integration:</b> {'✅ Installed' if XLWINGS_AVAILABLE else '❌ Not installed'}\n"
...
def main():
    ...
    print(f"✅ xlwings: {'✅ INSTALLED' if XLWINGS_AVAILABLE else '❌ NOT INSTALLED'}")
    ...
    try:
        ...
        # Remove potentially locked file
        if LOCAL_COPY_PATH.exists():
            try:
                os.remove(LOCAL_COPY_PATH)
                print("✅ Removed potentially locked file")
            except:
                print("⚠️ Could not remove file (may be in use)")
    except:
        pass
    ...
    if XLWINGS_AVAILABLE:
        ...
        print("⚠️  xlwings not installed. Please install to use full features:")
