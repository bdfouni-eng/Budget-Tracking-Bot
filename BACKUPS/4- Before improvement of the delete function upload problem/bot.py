...
def upload_to_onedrive() -> Tuple[bool, str]:
    """Upload local Excel file to OneDrive using Microsoft Graph API with retry on lock."""
    try:
        token = get_onedrive_token()
        if not token:
            return False, "❌ Not authenticated with OneDrive. Use /onedrive_auth first."
        ...
        # Retry on 423 Locked and 409 Conflict
        max_retries = 5
        ...
        for attempt in range(1, max_retries + 1):
            try:
                logger.info(f"Uploading to OneDrive attempt {attempt}/{max_retries}: {LOCAL_COPY_PATH}")
                ...
                # Graph may return JSON with error info
                try:
                    err_msg = response.json().get('error', {}).get('message', response.text)
                except Exception:
                    err_msg = response.text

                # 423 Locked or 409 Conflict or 429 Too Many Requests -> retry
                if response.status_code in (423, 409, 429):
                    ...
                    logger.warning(f"Upload locked or busy. Will retry in {sleep_s:.1f}s. Error: {last_err}")
                    time.sleep(sleep_s)
                    continue

                # Other errors are not retried
                last_err = f"{response.status_code} {err_msg}"
                logger.error(f"Upload error (no retry): {last_err}")
                return False, f"❌ Upload error: {err_msg}"

            except Exception as inner:
                last_err = str(inner)
                logger.warning(f"Upload attempt {attempt} failed with exception: {inner}")
