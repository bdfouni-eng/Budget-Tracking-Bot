...
async def onedrive_test_command(update, context):
    ...
    # Run blocking download/upload in a thread
    try:
        success, msg = await asyncio.to_thread(copy_excel_from_onedrive)
        await update.message.reply_text(f"Download test: {msg}")

        if success:
            # upload_to_onedrive is blocking — run it in a thread
            success2, msg2 = await asyncio.to_thread(upload_to_onedrive)
...
def upload_to_onedrive_direct() -> Tuple[bool, str]:
    """Simple direct upload without sessions"""
    try:
        token = get_onedrive_token()
        if not token:
        ...
        logger.info(f"Direct upload to: {ONEDRIVE_FILE_PATH}")
        ...
        if response.status_code in (200, 201):
            ...
            if "busy" in error_msg.lower() or "423" in error_msg or "same name" in error_msg.lower():
                ...
                try:
                    ...
                    # Log but don't raise; we still want to inform the caller that upload couldn't proceed now
                    logger.warning(f"Error while attempting to schedule background sync: {e}")

                return False, "⚠️ OneDrive is busy. Background sync queued."
        ...
        with excel_lock:
            ...
            try:
                for tbl in sheet.tables.values():
                    ...
                    header_vals = [ (sheet.cell(row=min_row, column=c).value or "").strip().lower() for c in range(min_col, max_col+1) ]
...
def save_excel_safely(filepath: Path, sheet_name: str, row: int, values: dict) -> bool:
    ...
    This function ensures the file remains a valid .xlsm file.
    """
    try:
        # Create a temporary file
        temp_path = filepath.with_suffix('.temp.xlsm')
        original_path = filepath.with_suffix('.backup.xlsm')

        # Backup original
        shutil.copy2(filepath, original_path)

        # Load workbook with keep_vba to preserve macros
        wb = load_workbook(str(filepath), keep_vba=True)
        ws = wb[sheet_name]

        # Clear data validation for cells being modified
        # This prevents validation errors for empty cells
        for cell in values.keys():
            # Clear any existing data validation for this cell
            # We need to check if this cell has data validation
            if ws.data_validations:
                # Get all data validations that apply to this cell
                cell_dv = None
                for dv in ws.data_validations.dataValidation:
                    # Check if this data validation applies to our cell
                    if dv.sqref:
                        ranges = str(dv.sqref).replace('$', '').split()
                        for r in ranges:
                            if ':' in r:
                                # Range like A1:A10
                                start, end = r.split(':')
                    ...
                    # If we found a data validation for this cell, clear it
                    if cell_dv:
                        # Remove this cell from the data validation range
                        # We'll create a new range without this cell
                        ranges = str(cell_dv.sqref).replace('$', '').split()
                        new_ranges = []
                        for r in ranges:
                            if ':' in r:
                                # It's a range, we need to check if our cell is in it
                                start, end = r.split(':')
                                start_row = int(''.join(filter(str.isdigit, start)))
                                end_row = int(''.join(filter(str.isdigit, end)))
                                cell_row = int(''.join(filter(str.isdigit, cell)))

                                if start_row <= cell_row <= end_row:
                                    # Our cell is in this range, split the range
                                    if cell_row == start_row and cell_row == end_row:
                                        # Single cell range, just skip it
                                        continue
                                    elif cell_row == start_row:
                                        # Cell is at start, adjust start
                                        new_start = f"{start[0]}{start_row + 1}"
                                        new_ranges.append(f"{new_start}:{end}")
                                    elif cell_row == end_row:
                                        # Cell is at end, adjust end
                                        new_end = f"{end[0]}{end_row - 1}"
                                        new_ranges.append(f"{start}:{new_end}")
                                    else:
                                        # Cell is in middle, split into two ranges
                                        range1_end = f"{end[0]}{cell_row - 1}"
                                        range2_start = f"{start[0]}{cell_row + 1}"
                                        new_ranges.append(f"{start}:{range1_end}")
                                        new_ranges.append(f"{range2_start}:{end}")
                                else:
                                    # Cell not in this range, keep as is
                                    new_ranges.append(r)
                            else:
                                # Single cell, skip if it's our cell
                                if r != cell:
                                    new_ranges.append(r)

                        if new_ranges:
                            cell_dv.sqref = ' '.join(new_ranges)
                        else:
                            # No ranges left, remove this data validation
                            try:
                                ws.data_validations.dataValidation.remove(cell_dv)
                            except Exception:
                                pass
        ...
        if temp_path.stat().st_size > 1000:
            ...
            # Temp file is corrupted, restore from backup
            logger.error("Temp file too small, restoring from backup")
            shutil.copy2(original_path, filepath)
            return False

    except Exception as e:
        logger.error(f"Error in save_excel_safely: {e}")
        return False
...
async def recent_command(update, context):
    ...
    try:
        # Download / ensure file in background
        success, msg = await asyncio.to_thread(copy_excel_from_onedrive)
        if not success:
            await update.message.reply_text(msg)
            return

        # Run the workbook reading in a thread to avoid blocking event loop
        def _read_recent():
            recent = []
            try:
                with excel_lock:
                ...
                logger.exception("Error reading recent transactions")
            return recent
...
async def handle_message(update, context):
    ...
    # Check for optional notes on new line
    ...
    # Parse the transaction part (parsing is lightweight)
    ...
    # Run the actual add_transaction_smart in a thread (it is blocking and uses excel_lock internally)
    try:
        success, message = await asyncio.to_thread(add_transaction_smart, subcategory, currency_amounts, payment_type, optional_notes)
        await update.message.reply_text(message, parse_mode='HTML')
    except Exception as e:
...
async def save_command(update, context):
    ...
    try:
        # save_excel_to_onedrive is blocking; run in a thread
        success, message = await asyncio.to_thread(save_excel_to_onedrive)
...
async def force_sync_command(update, context):
    """Force immediate sync with OneDrive"""
    if not is_authorized(update.effective_user.id):
        await update.message.reply_text("⛔ Unauthorized.")
        return
    ...
    if success:
        ...
        # Try one more time
        time.sleep(2)
        success2, msg2 = save_excel_to_onedrive()
        
        if success2:
            await update.message.reply_text(f"✅ Sync succeeded on second attempt!")
        else:
            await update.message.reply_text(
                f"❌ Sync failed twice:\n\n"
                f"First attempt: {msg}\n\n"
